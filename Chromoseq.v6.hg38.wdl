workflow ChromoSeq {
  ...
  String Cram    # = "/gscmnt/gc2555/spencer/dhs/data/work_orders/wgs/2856172-2856594/H_KA-692900G-S.9045_1-lib1/H_KA-692900G-S.9045_1-lib1.bam"
  String CramIndex     # = "/gscmnt/gc2555/spencer/dhs/data/work_orders/wgs/2856172-2856594/H_KA-692900G-S.9045_1-lib1/H_KA-692900G-S.9045_1-lib1.bam.bai"
  String Name
  String OutputDir

  String Translocations = "/gscmnt/gc2555/spencer/dhs/projects/chromoseq/hg38_files/ChromoSeq.translocations.fixed.v2.sorted.hg38.bedpe"
  String Cytobands = "/gscmnt/gc2555/spencer/dhs/projects/chromoseq/hg38_files/ChromoSeq.hg38.bed"

  String CoverageBed = "/gscmnt/gc2555/spencer/dhs/projects/chromoseq/hg38_files/CoverageQC.hg38.083118.bed"

  String VEP = "/gscmnt/gc2709/info/production_reference_GRCh38DH/CLE/IDTExome/VEP_cache/"
  
  Float minVarFreq=0.05
  
  String MantaConfig = "/gscmnt/gc2555/spencer/dhs/projects/chromoseq/configManta.hg38.py.ini"
...
task run_manta {
     ...
     command <<<
	     ...
	     ./manta/runWorkflow.py -m local -q research-hpc -j 8 -g 32 && \
	     ...
	     perl /usr/local/bin/BlatContigs.pl -r ${Reference} ${Name}.tumorSV.vcf.gz ${Name}.tumorSV.filtered.vcf && \
     ...
     output {
	    File tumor_vcf = "${Name}.tumorSV.vcf.gz"
	    File tumor_vcf_index = "${Name}.tumorSV.vcf.gz.tbi"
...
task run_ichor {
     ...
     command <<<
       ...
       --normalPanel /usr/local/lib/R/site-library/ichorCNA/extdata/HD_ULP_PoN_hg38_500kb_median_normAutosome_median.rds \
       ...
       /usr/local/bin/bedtools groupby -g 1,2,3,4,5,6,7 -c 12,12,13,13,14 -o distinct,count_distinct,distinct,count_distinct,distinct >> "${Name}.cnv_report.txt"; fi && \
...
task run_platypus {
     ...
     command <<<
       ...
       /opt/conda/bin/octopus -R ${refFasta} -I ${Bam} -t regions.txt -C cancer | /bin/sed 's/VCFv4.3/VCFv4.1/' > "${Name}.platypus.vcf"
     >>>
...
task make_bw {
  ...
  command {
    ...
    --ignoreDuplicates -bl /opt/files/hg38.blacklist.merged.bed --binSize 50 --minMappingQuality 1 --extendReads -p 4 -ignore X Y MT
  }
  runtime {
    docker_image: "dhspence/docker-chromoseq"
    cpu: "4"
...
task annotate_svs {
  ...
  command {
    /usr/bin/perl -I /opt/lib/perl/VEP/Plugins /usr/bin/variant_effect_predictor.pl \
    --format vcf --vcf --plugin Downstream --plugin Wildtype --fasta ${refFasta} --symbol --term SO --pick -o ${Name}.svs_annotated.vcf \
...
task make_igv {
  ...
  command {
    ...
    <Resource name="${Name}" path="${Name}.cnv.bed"/>
