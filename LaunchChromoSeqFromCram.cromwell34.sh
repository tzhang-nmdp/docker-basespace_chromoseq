...
LSF_PRESERVE_ENVIRONMENT=false bsub -g /dspencer/chromoseq -oo $OUTDIR/out.$NAME.log -eo $OUTDIR/err.$NAME.log -q research-hpc -a "docker(registry.gsc.wustl.edu/apipe-builder/genome_perl_environment:7)" /usr/bin/java -Dconfig.file=/gscmnt/gc2555/spencer/dhs/projects/wdltest/application.new.conf -jar /opt/cromwell.jar run -t wdl -i $OUTDIR/chromoseq.$NAME.json /gscuser/dspencer/projects/chromoseq/Chromoseq.v8.hg38.wdl
